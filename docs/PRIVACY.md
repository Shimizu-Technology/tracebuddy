...
Saved-work metadata, strokes, tool settings, custom text, local image references, favorite template IDs, up to eight recent template IDs, guided-lesson step/completion IDs, whether the parent setup coach has been completed, and one saved overlay alignment may persist on the device until local work is cleared, site data is cleared, or the mobile app is removed. A saved alignment contains only the overlay position, size, rotation, and opacity; it does not contain a camera image. The web app uses `localStorage` and IndexedDB. The native app uses AsyncStorage and app-private files.
